...
class SimpleDbStore:
    ...
    def create_store_sql(self):
        ...
        self.store_sql = f"INSERT OR IGNORE INTO {self.map.db_table} ({', '.join(self.db_fields)}) VALUES ({placeholders});"
        self.fetch_by_id_sql = f"SELECT {self.map.id_field}, {', '.join(self.db_fields)} FROM {self.map.db_table} WHERE {self.map.id_field} = ?"

    def create_store_sql_pg(self):
        self.db_fields = list(self.map.item_to_db.keys())
    ...
    def store_item(self, item: Item) -> Item:
        ...
        with self.db.cursor() as cursor:
            ...
            if new_item_id:
                item[self.map.id_field] = new_item_id
                return item
        return None            # raise ValueError("Failed to retrieve new item ID after insertion.")
...
class KeyedDbStore(SimpleDbStore, Generic[KeyType]):
    ...
    def create_store_sql(self):
        ...
        self.fetch_sql = f"SELECT {self.map.id_field}, {', '.join(self.db_fields)} FROM {self.map.db_table} WHERE {self.map.key_field} = ?"
